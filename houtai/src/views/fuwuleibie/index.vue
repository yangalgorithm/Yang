		...
		<el-table size="small" :data="listData" highlight-current-row v-loading="loading" border element-loading-text="拼命加载中" @selection-change="handleSelectionChange">
			...
			<el-table-column align="center" label="操作" min-width="160">
				<template slot-scope="scope">
					...
					<el-button size="mini" type="danger" @click="deleteFuwuleibie(scope.$index, scope.row)" v-if="userInfo.roles[0] == 'guanliyuan' ">删除</el-button>
...
export default {
	...
	methods: {
		...
		//zxhifuff
		handleShenheList(){
				const fuwuleibies = this.fuwuleibies;
				if(fuwuleibies.length == 0){
					this.$message({
						type: 'error',
						message: '请至少选择一项'
					})
					...
					this.$confirm('确定要批量审核吗?', '信息', {
						...
						fuwuleibies.forEach(item=> {
							let data = {...item, issh: 1}
							fuwuleibieEdit(data).then(res => {})
						})
						this.getdata(this.formInline)
						this.$message({
							type: 'success',
		...
		submitForm(editData) {
			...
			this.$refs[editData].validate(valid => {
				if (valid) {
					if(this.editForm.id){
						fuwuleibieEdit(this.editForm).then(res => {
						...
						fuwuleibieSave(this.editForm).then(res => {
							this.editFormVisible = false
							this.loading = false
							if (res.code == '0') {
		...
		// 删除公司
		deleteFuwuleibie(index, row) {
			this.$confirm('确定要删除吗?', '信息', {
				confirmButtonText: '确定',
				cancelButtonText: '取消',
				type: 'warning'
			}).then(() => {
				fuwuleibieDelete(row.id).then(res => {
					if (res.code == '0') {
						this.$message({
								type: 'success',
							message: '服务类别已删除!'
		...
		handleDeleteList(){
			...
			if(fuwuleibies.length == 0){
				...
				this.$confirm('确定要批量删除吗?', '信息', {
					...
					fuwuleibieDeleteList(fuwuleibies).then(res => {
						if (res.code == '0') {
							this.$message({
								type: 'success',
								message: '批量删除成功!'
