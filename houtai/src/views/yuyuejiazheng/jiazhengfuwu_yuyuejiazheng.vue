	...
	<div>
		...
		<el-table size="small" :data="listData" highlight-current-row v-loading="loading" border element-loading-text="拼命加载中" @selection-change="handleSelectionChange">
			...
			<el-table-column align="center" label="操作" min-width="160">
				<template slot-scope="scope">
					<el-button size="mini"  type="danger" @click="addSalary(scope.$index, scope.row)">添加汽车预约订单</el-button>
				</template>
			</el-table-column>
		</el-table>
...
import { jiazhengfuwuList, jiazhengfuwuSave, jiazhengfuwuDelete,jiazhengfuwuEdit,jiazhengfuwuDeleteList } from '@/api/jiazhengfuwu/jiazhengfuwuApi';
...
export default {
	...
	methods: {
		...
		getAllfuwuleibie(){fuwuleibieAllList().then(res=>{if(res.code == '0'){this.fuwuleibies = res.data}})},
