<template>
  <div id="details">
    ...
    <div class="main">
		<el-card  shadow="hover" style="width: 560px; float:left">
			<div class="block">
				<div>
					<img style="height:500px;width: 500px" :src="`/api/`+jiazhengfuwudetail.tupian" />
				</div>
			</div>
		</el-card>
      <div class="content">
  <table width="100%" border="0"  align="center" cellpadding="3" cellspacing="1" class="newsline">
								<tr>
        <td width='22%' height=44>服务名称：</td><td width='39%' >{{jiazhengfuwudetail.fuwumingcheng}}</td></tr><tr>
         <td width='22%' height=44>类别：</td><td width='39%' >{{jiazhengfuwudetail.leibie}}</td></tr><tr>
         <td width='22%' height=44>服务价格：</td><td width='39%' >{{jiazhengfuwudetail.fuwujiage}}</td></tr><tr>
         <td width='22%' height=44>服务时长：</td><td width='39%' >{{jiazhengfuwudetail.fuwushichang}}</td></tr><tr>


         <td width='22%' height=44>工号：</td><td width='39%' >{{jiazhengfuwudetail.gonghao}}</td></tr><tr>
         <td width='22%' height=44>名字：</td><td width='39%' >{{jiazhengfuwudetail.mingzi}}</td></tr><tr>
         <td width='22%' height=100  >服务简介：</td><td width='39%' colspan=2 height=100 >{{jiazhengfuwudetail.fuwujianjie}}</td></tr>
		</tr>
</table>
        <div class="button">
		  <el-button class='shop-cart' @click='addCollect'>喜欢</el-button>
		  <el-button class='like' @click='handleEdit()'>我有话说</el-button>
      <el-button class='like' @click='wyyy()' style="background: blue">预约服务</el-button>
...
<script>
  ...
  export default {
    ...
    created() {
      this.jiazhengfuwuID= this.$route.params.id;
    ...
    methods: {
      ...
      wyyy()
      {
        ...
        let id = this.$route.params.id;
        this.$router.push('/yuyuejiazhengadd/'+id)
      },
      ...
      handleEdit: function () {
                ...
                this.title = '添加评论'
                this.editForm.biao="jiazhengfuwu";
                this.editForm.shujuid=this.jiazhengfuwuID;
                this.userInfo = JSON.parse(localStorage.getItem("user"));
                this.editForm.pinglunren= localStorage.getItem("username");
            },
            getpinglun() {
                this.$axios
                    .post("/api/front/pinglun", {biao:"jiazhengfuwu",shujuid:this.jiazhengfuwuID})
                    .then(res => {
      ...
      getDetails(val) {
        this.$axios
                .get("/api/front/getjiazhengfuwudetail/"+val,)
                .then(res => {
                  debugger
                  this.jiazhengfuwudetail = res.data.data;
                })
                .catch(err => {
                  return Promise.reject(err);
                });
        ...
        this.$axios
                .post("/api/front/addCollect", {
                  ...
                  shangpinxinxiID: this.jiazhengfuwuID
                })
                .then(res => {
                  if (res.data.code == "0") {
